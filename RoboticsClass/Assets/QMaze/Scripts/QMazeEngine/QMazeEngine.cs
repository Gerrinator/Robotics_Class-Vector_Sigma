...
namespace qtools.qmaze
{
	...
	public class QMazeEngine : MonoBehaviour 
	{
		...
		[SerializeField] private List<QVector2Int> pathPositionList = new List<QVector2Int>();
		...
		public List<QVector2Int> Pathfinder(int startPositionX, int startPositionY, int finishPositionX, int finishPositionY)
        {
			path = new List<QVector2Int>();
			QVector2Int currentPosition = new QVector2Int(startPositionX, startPositionY);

			return path;
        }

		public IEnumerator generate(bool generateWithGeometry = true, bool asynchronous = true, float maxTime = 0.1f)
		{
			...
			//pathlist.Add(new List<int>() { currentPosition.x, currentPosition.y });
			...
			do
			{
				//trying to track down the path...
				//Debug.Log("Tracking: "  + currentPosition.x + " " + currentPosition.y);
				...
				if (lastDirection == QMazeOutputDirection.NotSpecified)
				{
					...
					//pathlist.RemoveAt(path.Count - 1);
				}
		...
		public List<QVector2Int> getPath(){
			return path;
		}
